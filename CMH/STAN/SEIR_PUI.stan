...
data {
    ...
    real<lower=0> obs_Hpui[nhobs];  // observations num non-icu hospitalized persons under investigation (pui); this is a vector
    real<lower=0> obs_Hicu[nhobs];  // observations num icu hospitalized; this is a vector
    real<lower=0> obs_Hpui_icu[nhobs];  // observations num icu hospitalized persons under investigation (pui); this is a vector
    ...
    real<lower=0.0> mu_pui_pos;         // mean fraction of pui who are positive
    real<lower=0.0> sigma_pui_pos;      // sd fraction of pui who are positive
...
parameters {
    ...
    real<lower=0, upper=1> frac_tested;
    real<lower=0, upper=1> frac_puipos;
...
transformed parameters {
    ...
    real Rt[nt];
    {
        ...
        for (it in 1:nt-1){
            ...
            mu_Iobs[it+1] = mu_Iobs[it] + newI_sum * frac_tested;
...
model {
    ...
    frac_puipos ~ normal(mu_pui_pos, sigma_pui_pos); 
    ...
    sigma_Hicu ~ exponential(1.0);
    {
        ...
        for (iobs in 1:nhobs){
            tmp = (obs_Hmod[iobs]+frac_puipos*obs_Hpui[iobs]-x[Hmod,thobs[iobs]])/sigma_Hmod;
            tmp ~ std_normal();

            tmp = (obs_Hicu[iobs]+frac_puipos*obs_Hpui_icu[iobs]-x[Hicu,thobs[iobs]])/sigma_Hicu;
            tmp ~ std_normal();
        }
    }
}
generated quantities{
    ...
    for (it in 1:nt){
        ...
        // fracpuipos[it] = hospitalized[it]/(obs_Hmod[it]+obs_Hpui[it]+obs_Hicu[it]+obs_Hpui_icu[it]);
