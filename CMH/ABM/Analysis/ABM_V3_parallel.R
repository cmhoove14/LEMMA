...
t.end <- as.Date("2020-11-01")       # Simulation end date
...
start_Es <- c(rep(10, n_sims/2), rep(20, n_sims/2))

cl <- makeCluster(n_cores)
clusterExport(cl, c("agents", "sf_cbg_cdf_expand", "n_sims", "N", "sf_cbg_ids", "sf_sfgrph_pct_home_expand",
                    "SiP.start", "scl.close", "mask.start", "start_Es", "trans.all", 
                    "day_of_week_expand", "time_of_day", "t.tot", "dt", "t0"))
invisible(clusterEvalQ(cl, lapply(c("data.table", "wrswoR", "dqrng", "matrixStats", "fastmatch", "lubridate"), 
                        library, character.only = T)))
invisible(clusterEvalQ(cl, source("CMH/ABM/R/ABM_V3.R")))

sim_results <- parLapply(cl, start_Es, 
                         fun = function(x) covid_abm_v3_no_test(bta_hh = trans.all, bta_scl = trans.all, bta_work = trans.all, bta_other = trans.all,       
                                                                E0 = x, Ip0 = 5, Ia0 = 5, Im0 = 1, Imh0 = 0, Ih0 = 0, R0 = 0, D0 = 0,                  
                                                                agents_dt = agents, cbg_cdf_array = sf_cbg_cdf_expand, cbg_ids = sf_cbg_ids, 
                                                                stay_home_dt = sf_sfgrph_pct_home_expand,          
                                                                t0 = t0, t.tot = t.tot, dt = dt, day_of_week_fx = day_of_week_expand, time_of_day_fx = time_of_day, 
                                                                SiP.start = SiP.start, scl.close = scl.close, mask.start = mask.start, mask.red = 0.6,
...
saveRDS(sim_results, paste0("CMH/ABM/Analysis/Outputs/ABM_V3_no_tests",Sys.Date(),".rds"))
